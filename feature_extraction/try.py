...
file_name = str(path)+f"/feature_extraction/user_capture/{file_prefix}.pcap"
scapy.wrpcap(file_name, captured_packets) #抓包日志
scapy.wrpcap(str(path)+"/feature_extraction/user_packet.pcap", captured_packets) #固定文件，存储每次抓包的数据，覆盖写入
...
# 提取特征
str1 = str(path)+r"\CICFlowMeter-4.0\bin" #脚本目录
str2 = str(path)+r"\feature_extraction\user_packet.pcap" #固定存包
str3 = str(path)+r"\feature_extraction\trans_csv"  #特征csv存入目录
get_csv(str1, str2, str3)

# 处理csv
# 读取特征csv文件
# 这里的编码可能出错
df = pd.read_csv(str(path)+"/feature_extraction/trans_csv/user_packet.pcap_Flow.csv") #默认user_packet.csv
...
# 保存最终可用的csv文件
outputpath = str(path)+'/feature_extraction/data_new.csv'
